# This code saves the images and also let the viewr see the area, and the percentage of the area the voids take up of the total area of the solder balls in the terminal it is shown as output.
import cv2
import numpy as np
import os
...
def analyze_bga_complete(image_path, annotation_path, output_folder):
    ...
    while True:
        ...
        if key == 13:  # Enter
            threshold_val += 1
        elif key == 8 or key == 255:  # Backspace
            threshold_val -= 1
    ...
    # --- FINAL PROCESSING, SAVING & TERMINAL REPORT ---
    final_img = img.copy()
    print("\n--- VOID ANALYSIS RESULTS ---")
    print(f"{'Ball':<12} {'Void Area (px²)':<20} {'Ball Area (px²)':<20} {'Void %':<10}")
    print("-" * 65)
    ...
    for i, line in enumerate(lines):
        ...
        # Ball mask — total area of solder ball
        _, bm = cv2.threshold(rg, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)

        # Void mask — bright regions inside ball
        ...
        # Area calculations
        total_ball_area = cv2.countNonZero(bm)
        total_void_area = sum(cv2.contourArea(c) for c in cnts if cv2.contourArea(c) > 2)
        void_percentage = (total_void_area / total_ball_area) * 100 if total_ball_area > 0 else 0.0

        # Terminal output
        print(f"Solder ball {i+1:<4} Void area: {int(total_void_area):<16} Total ball area: {total_ball_area:<16} Void %: {void_percentage:.2f}%")

        # Draw contours and save
        cv2.drawContours(roi, cnts, -1, (0, 0, 255), 1)
        final_img[y1:y2, x1:x2] = roi
        cv2.imwrite(os.path.join(output_folder, f"ball_{i+1}.jpg"), roi)

    print("-" * 65)
    cv2.imwrite(os.path.join(output_folder, "A_FULL_BOARD_RESULT.jpg"), final_img)
    cv2.destroyAllWindows()
    print(f"\nSuccessfully saved all images to: {output_folder}")


# Paths
input_image = "/home/oem/Documents/Intel/for_img_35_DOC1_leg1/train/images/35.jpg"
input_txt = "/home/oem/Documents/Intel/for_img_35_DOC1_leg1/train/labels/35.txt"
output_dir = "/home/oem/Documents/Intel/result_img35"
